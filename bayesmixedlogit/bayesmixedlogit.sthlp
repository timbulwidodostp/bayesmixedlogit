{smcl}
{* 2Feb2013}{...}
{cmd:help bayesmixedlogit}{right: ({browse "http://www.stata-journal.com/article.html?article=st0354":SJ14-3: st0354})}
{hline}
...
posterior parameter distribution and then presents a summary and other
...
coefficients is taken to be an identity inverse Wishart.  {cmd:bayesmixedlogit} uses the Mata routines {helpb mf_amcmc:amcmc()} (if not
...
{phang}
{opt draws(#)} specifies the number of draws that are to be taken from the posterior distribution of the parameters.  The default is {cmd:draws(1000)}.
...
{phang2}{cmd:. bayesmixedlogit choice, rand(YesXurban YesXage Yes) group(_id) id(district) draws(10000) burn(5000) saving(bdesh_draws) replace}
...
{phang}
Hole, A. R. 2007.
{browse "http://www.stata-journal.com/article.html?article=st0133":Fitting mixed logit models by using maximum simulated likelihood}.
{it:Stata Journal} 7: 388-401.

{phang}
Long, J. S., and J. Freese. 2006.
{browse "http://www.stata.com/bookstore/regression-models-categorical-dependent-variables/":{it:Regression Models for Categorical Dependent Variables Using Stata}}.
2nd ed. College Station, TX: Stata Press. 

{phang}
Train, K. E. 2009.
{it:Discrete Choice Methods with Simulation}.
